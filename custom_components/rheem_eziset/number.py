...
class RheemEziSETNumber(RheemEziSETEntity, NumberEntity):
    ...
    def native_value(self):
        ...
        if self.rheem_session_timer is not None:
            ...
            return (self.coordinator.data or {}).get("sessionTimer")

    async def async_set_native_value(self, value: float) -> None:
        """Set the session timer of the water heater."""
        api = self.coordinator.api
        session_timer = self.rheem_session_timer = value
        await api.async_set_session_timer(number=self, session_timer=session_timer)
        await self.coordinator.async_schedule_fast_refresh("set_session_timer")